...
def window_partition(x, window_size):
    ...
    for i in range(n_contrast):
        ...
        window_i = x_i.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, 1, window_size[0], window_size[1],
                                                                   C)  # (num_windows*B, 1, window_size, window_size, C)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_q, x_k, x_v, mask=None, return_attention=False):
        ...
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous().repeat(1, n_contrast_q,
                                                                                             n_contrast_kv)  # nH, Wh*Ww*c_q, Wh*Ww*c_kv
        attn = attn + relative_position_bias.unsqueeze(0)

        if mask is not None:
            # mask: # nW, window_size**2, window_size**2
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N_q, N_kv) + mask.repeat(1, n_contrast_q,
                                                                                    n_contrast_kv).unsqueeze(
                1).unsqueeze(0)
